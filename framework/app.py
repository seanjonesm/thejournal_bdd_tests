...
class App(object):
    ...
    def select_article_by_number(self, article_number):
        try:
            articles = self._driver.find_elements_by_xpath('//div[contains(@class, "post")]/div[2]/h4/a')
            articles[(article_number - 1)].click()
        except Exception as e:
            self._log.Write('Error: ' + str(e))
    ...
    def verify_author_name_populated(self):
        ...
        assert  len((author_name.text.strip())) > 0, "Author name not populated"
